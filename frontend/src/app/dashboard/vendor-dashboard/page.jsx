...
// Sidebar Hook
export const useSidebar = (initialExpandedCategories = { Core: true }) => {
  ...
  const filterGroupedItems = (groupedItems) => {
...
// Dashboard Hook
export const useDashboard = (recentSales, orders) => {
...
export const SearchBar = ({ searchQuery, setSearchQuery, isCollapsed }) => {
  if (isCollapsed) return null;

  return (
    <div className="relative mb-4 px-6">
      <Search className="w-4 h-4 absolute left-9 top-1/2 transform -translate-y-1/2 text-gray-400" />
      <input
        type="text"
...
export const NavigationMenu = ({
  ...
  <nav className="flex-1 px-3 pb-4 overflow-y-auto space-y-3 custom-scrollbar">
      ...
      filteredGroupedItems[category] && (
        <div key={category} className="group">
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-2 rounded-lg hover:bg-gray-700/50 transition-all group"
          >
    ...
    {searchQuery && Object.keys(filteredGroupedItems).length === 0 && !isCollapsed && (
...
export const EnhancedSidebar = ({ activeTab, setActiveTab, categoryOrder, groupedItems, sidebarItems }) => {
  const {
    expandedCategories,
    searchQuery,
    isCollapsed,
  ...
  const filteredGroupedItems = filterGroupedItems(groupedItems);
  ...
  return (
    <div className={`${isCollapsed ? 'w-20' : 'w-64'} bg-gradient-to-b from-[#2C2C2C] to-[#1F1F1F] text-white flex flex-col overflow-hidden transition-all duration-300 ease-in-out shadow-2xl`}>
      ...
      <SearchBar searchQuery={searchQuery} setSearchQuery={setSearchQuery} isCollapsed={isCollapsed} />
      <NavigationMenu
...
// Stat Card Component
export const StatCard = ({ title, value, change, trend, icon: Icon, color, onClick, selectedMetric }) => (
  <div
    onClick={onClick}
...
export const DashboardHeader = ({ isRefreshing, handleRefresh }) => (
...
export const SearchFilterBar = ({ searchQuery, setSearchQuery, dateRange, setDateRange }) => (
...
// Bar Chart Component
export const BarChart = ({ barData, hoveredBar, setHoveredBar }) => (
  <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-shadow">
    ...
    <div className="flex items-end justify-between h-56 gap-3 mb-4">
      {barData.map((bar, index) => (
        ...
        >
          {hoveredBar === index && (
            <div className="absolute -top-12 bg-gray-900 text-white px-3 py-2 rounded-lg text-xs whitespace-nowrap animate-fade-in">
    ...
    <div className="relative w-32 h-32 mx-auto mb-6">
      <svg className="w-full h-full transform -rotate-90 drop-shadow-lg hover:drop-shadow-2xl transition-all">
        ...
        <circle
          ...
          strokeDasharray="220"
          strokeDashoffset="55"
          className="transition-all duration-500 hover:stroke-width"
        />
        <defs>
...
export const RecentSales = ({ filteredSales }) => (
...
export const OrdersTable = ({ filteredOrders }) => (
  <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-shadow">
    <div className="flex justify-between items-center mb-6">
      <div>
...
const EnhancedDashboardContent = ({ barData, recentSales, orders }) => {
  const {
    activeTab,
    dateRange,
    searchQuery,
    ...
    setHoveredBar,
    handleRefresh
  } = useDashboard(recentSales, orders);
    ...
    <div className="flex-1 overflow-auto bg-gradient-to-br from-[#F8F9FB] to-[#E8E6DC]">
      <div className="p-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <BarChart barData={barData} hoveredBar={hoveredBar} setHoveredBar={setHoveredBar} />
          <DonutChart />
        </div>
