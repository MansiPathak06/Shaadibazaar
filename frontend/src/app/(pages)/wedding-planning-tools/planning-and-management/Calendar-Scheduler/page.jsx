...
import { ChevronLeft, ChevronRight, Plus, Calendar, LayoutDashboard, X, Trash2, Clock, MapPin, User, Tag, AlignLeft, Share2, ArrowLeft, Loader2, AlertCircle } from 'lucide-react';
...
const apiFetch = async (path, options = {}) => {
    // Pull JWT from localStorage — same key your other frontend calls use
...
const EventProvider = ({ children }) => {
    ...
    // Load all events for the logged-in user on mount
    ...
    const addEvent = async (newEvent) => {
        ...
        const optimistic = { ...newEvent, id: tempId };
        ...
        try {
            ...
            setEvents(prev => prev.map(e => e.id === tempId ? data.data : e));
        } catch (err) {
            // Rollback on error
    ...
    const updateEvent = async (updatedEvent) => {
        ...
        try {
            await apiFetch(`/api/calendar/${updatedEvent.id}`, {
                method: 'PUT',
...
// ─── Constants ────────────────────────────────────────────────────────────────
const CATEGORIES = [
    'Vendor Meetings', 'Dress Fittings', 'Payment Due Dates',
    'Venue Visits', 'Personal Appointments', 'Other'
];
...
const EventModal = ({ isOpen, onClose, onSave, onDelete, initialDate, eventToEdit }) => {
    const [formData, setFormData] = useState({
        ...
        location: '', description: '', assignedTo: 'Bride', category: 'Other'
    });
        ...
        if (isOpen) {
            if (eventToEdit) {
                ...
                setFormData({
                    ...
                    description: '', assignedTo: 'Bride', category: 'Vendor Meetings'
                });
            }
        }
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="p-6 space-y-4">
                        <div>
                            ...
                            <input type="text" name="title" required placeholder="e.g., Cake Tasting" value={formData.title} onChange={handleChange}
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input type="text" name="location" placeholder="Venue Address or Link" value={formData.location} onChange={handleChange}
                    ...
                    <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex justify-between items-center">
                        {eventToEdit ? (
                            <button type="button"
                                onClick={() => { if (window.confirm('Delete this event?')) { onDelete(eventToEdit.id); onClose(); } }}
                        ...
                        <div className="flex gap-3">
                            <button type="button" onClick={onClose} className="px-4 py-2 rounded-lg text-slate-600 font-medium hover:bg-slate-200 transition-colors">
                                Cancel
                            </button>
                            <button type="submit" className="px-6 py-2 rounded-lg bg-pink-600 text-white font-medium hover:bg-pink-700 shadow-md hover:shadow-lg transition-all">
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setCurrentDate(new Date())} className="px-3 py-1 text-sm font-medium hover:bg-slate-100 rounded-md text-slate-600">
                        Today
                    </button>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div className="grid grid-cols-7 auto-rows-fr">
                        {calendarDays.map((day) => {
                            ...
                            return (
                                <div key={day.toString()} onClick={() => onAddEvent(day)}
                                    className={`min-h-[120px] p-2 border-b border-r border-slate-100 transition-colors relative group cursor-pointer
                                    ...
                                    <div className="flex justify-between items-start">
                                        ...
                                        <button onClick={(e) => { e.stopPropagation(); onAddEvent(day); }}
                                    ...
                                    <div className="mt-2 space-y-1">
                                        {dayEvents.slice(0, 3).map(event => (
                                            <button key={event.id} onClick={(e) => { e.stopPropagation(); onEditEvent(event); }}
...
const Dashboard = ({ onNavigate }) => {
    ...
    const eventsThisWeek = upcomingEvents.filter(event => isSameWeek(parseISO(event.date), today));
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                <button onClick={handleShare} className="flex items-center gap-2 text-pink-600 hover:bg-pink-50 px-3 py-2 rounded-lg transition-colors font-medium text-sm">
                    <Share2 size={18} /> Share Calendar
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-pink-100">
                    ...
                    <p className="text-3xl font-bold text-slate-800 mt-2">
                        {loading ? <Loader2 className="animate-spin text-pink-400" size={24} /> : upcomingEvents.length}
                    </p>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-pink-100">
                    <h3 className="text-slate-500 text-sm font-medium">This Week</h3>
                    <p className="text-3xl font-bold text-pink-500 mt-2">
                        {loading ? <Loader2 className="animate-spin text-pink-400" size={24} /> : eventsThisWeek.length}
                    </p>
                </div>
                <div className="bg-gradient-to-br from-pink-500 to-pink-600 p-6 rounded-xl shadow-md text-white flex flex-col justify-between">
                    ...
                    <button onClick={() => onNavigate('calendar')}
                        className="mt-4 bg-white/20 hover:bg-white/30 py-2 px-4 rounded-lg text-sm font-medium transition-colors w-full">
                        Go to Calendar
                    </button>
                </div>
            </div>
            ...
            <div>
                ...
                {loading ? (
                    <div className="flex justify-center items-center h-32"><Loader2 className="animate-spin text-pink-500" size={28} /></div>
                    ...
                    <div className="bg-white p-8 rounded-xl border border-dashed border-slate-300 text-center">
                        ...
                        <button onClick={() => onNavigate('calendar')} className="mt-4 text-pink-600 font-medium hover:text-pink-700">
                            ...
                            <div key={event.id} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow flex sm:items-center flex-col sm:flex-row gap-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-sm text-slate-500">
                                        <div className="flex items-center gap-1"><Clock size={14} /> {event.startTime} - {event.endTime}</div>
                ...
                <nav className="flex space-x-1 bg-slate-100 p-1 rounded-lg">
                    <button onClick={() => onNavigate('dashboard')}
                        className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium transition-all
                            ${currentView === 'dashboard' ? 'bg-white text-pink-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                        <LayoutDashboard size={16} />
                        <span className="hidden sm:inline">Dashboard</span>
                    </button>
                    <button onClick={() => onNavigate('calendar')}
                        className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium transition-all
                            ${currentView === 'calendar' ? 'bg-white text-pink-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                        <Calendar size={16} />
...
const WeddingScheduler = () => {
    ...
    const handleSaveEvent = (eventData) => {
        if (eventToEdit) updateEvent(eventData);
        else addEvent(eventData);
    };

    return (
        <Layout currentView={currentView} onNavigate={handleNavigate} onBack={handleBack}>
            {currentView === 'dashboard' && <Dashboard onNavigate={handleNavigate} />}
            {currentView === 'calendar' && <CalendarGrid onAddEvent={handleAddEvent} onEditEvent={handleEditEvent} />}
            <EventModal
                isOpen={modalOpen}
